...
| Final runtime data | Not committed | Included in runtime and dev-derived release bundles | `data/derived/visualization_facts.json` is the bundled web payload. It is the sole required runtime file listed by `data_package.json`. The intermediate files it bundles (`chapter_facts.json`, `constellation_wireframes.json`, `predicted_rolls.json`) are included in dev-derived bundles but are not fetched by the web layer. |
...
outside the public repo. The release workflows check out that private repo at
`main` by default when `BCF_PRIVATE_SOURCE_DEPLOY_KEY` is configured, then run
`scripts/hydrate_source_epub.py`.

Contributor fallback: if the private source repo is unavailable, regeneration
can proceed only when a compatible EPUB already exists at
`data/raw/Brocktons_Celestial_Forge.epub`. The hydration script is non-networked
and records source metadata at `data/raw/Brocktons_Celestial_Forge.source.json`.
...
1. The data-release workflow depends on either the maintainer private source
   checkout or an already-present contributor EPUB. It no longer downloads from
   FicHub during release regeneration.
...
3. `visualization_facts.json` is the single bundled web payload produced by
   `scripts/build_visualization_facts.py`. It bundles `chapter_facts.json`,
   `constellation_wireframes.json`, and `predicted_rolls.json` into one file
   that the web app fetches alongside the deployment manifest. The upstream
   pipeline intermediates (`chapter_facts.json`, `constellation_wireframes.json`,
   `predicted_rolls.json`) remain valid and are still produced — they are internal
   only and not fetched by the web layer.
