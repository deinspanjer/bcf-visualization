...
Parked on the long-lived branch **`parked/stage2-inference`**, deliberately removed from `main` so
it is not mistaken for a foundation. The measured transport findings — including that a stdio MCP
server loses a startup race to `claude -p`, and that subprocess exit 0 does not mean success — are
in that branch's `.planning/workstreams/curation/phases/04-*/04-RESEARCH.md`.
