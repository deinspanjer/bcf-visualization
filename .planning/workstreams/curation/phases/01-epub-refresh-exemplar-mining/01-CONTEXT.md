...
- **D-10 (Dre, 2026-07-26, mid-planning):** EPUB-01 groundwork is already done — Dre ran the private-source sync + hydrate outside this workflow (`data/private-source` is at tag `source-v20260726.1`, synced 2026-07-26 17:23; `data/raw/Brocktons_Celestial_Forge.epub` present). Plans must NOT re-run sync/hydrate as a work step; they start at **verification** of the refreshed state (D-09 checks). Only if verification finds the epub stale does the executor fall back to the existing sync flow, surfacing the discrepancy rather than silently re-syncing.
