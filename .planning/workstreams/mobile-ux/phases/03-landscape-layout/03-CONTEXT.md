...
- **D-22 Mid-gesture rotation: abort the drag, keep the cinematic.** An in-flight rail scrub is cancelled cleanly (the existing teardown already clears the frozen pan offset at `app.js:3864`); a playing roll cinematic continues from state, since it is driven by `wordPos` rather than by the DOM. Do not defer or block the layout swap while a pointer is down — an ignored physical rotation reads as a freeze.
...
- **D-25 Content follows the prototype's `FieldLog`** (`design/mobile-ux/prototype/panels.jsx`): a highlighted live roll (roll number, outcome, CP, perk name, jump) above a list of recent entries (chapter, outcome/CP, name). Not a mirror of desktop's readout content — desktop has far more width to spend.
...
- **D-27 Keep the truncated evidence quote** (~100 chars) on the live roll — it is the strongest tie back to the story text and the data already carries it. It MUST have a hard height clamp so a long quote cannot push the recent list out of the rail.
...
- **D-34 Generalize `attachMobilePortraitGestures` (`app.js:3852`) into one `attachMobileGestures` that branches on `layoutMode`.** One set of teardown slots, one place where the D-07 attach lifecycle and the frozen-pan reset live. Do NOT add a parallel landscape sibling — that duplicates the defensive teardown logic that D-07 and the drag-monotonicity fix both depend on.
