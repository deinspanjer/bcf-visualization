...
consistent with the primary source (the EPUB story text), and that
the two curator sources agree with each other on the chapter range
they share.

Checks:
  1. Chapter alignment - every chapter the curators list also exists
     in chapters.json.
  2. Roll-pace math - the original mechanic was "100 CP per 2000
     words". For chapters with rolls, rolls in that chapter should
     track cp_earning_word_count / 20 (CP-eligible only — perk
     footers and non-MC POV sections are excluded).
  3. Perks consistency - every perk acquired in rolls.json must exist
     in perks_catalog.json. Cosmetic mismatches (case, ASCII vs
     Unicode punctuation, source aliases) are distinguished from
     genuine gaps.
  4. Curator agreement - rolls.json (sonicyoash, chapters 1-75) vs
...
def check_chapter_alignment(chapters: dict, rolls: dict) -> list[str]:
    ...
    errors: list[str] = []
    chapter_nums = {c["chapter_num"] for c in chapters["chapters"]}
    curator_nums = {
        r["chapter_num"]
        for r in rolls["rolls"]
        if r["chapter_num"] and r["chapter_num"] != "0"
    }
    bad = curator_nums - chapter_nums - _NON_CHAPTER_LABELS
    if bad:
        errors.append(
            f"  curator references chapters not in chapters.json: {sorted(bad)}"
        )
    print(f"check 1 - chapter alignment:")
    print(f"  chapters.json:         {len(chapter_nums)}")
...
def check_roll_pace(chapters: dict, rolls: dict, chapter_facts: dict) -> list[str]:
    """For each chapter with rolls, expect ~ rolls * 2000 ≈ CP-eligible words.

    Uses chapter_facts.json's cp_earning_word_count (derived from
    section_classifications.json) rather than the raw chapter total —
    the "100 CP per 2000 words" rule is about narrative prose, not
    perk footers, author notes, or non-MC POV interludes.

    Returned issues are SOFT - the rule is a fictional in-story
    mechanic, not a hard math invariant. Edits, interlude pacing, and
    curator omissions all create real noise.
    """
    errors: list[str] = []
    cp_words_by_num = {
        c["chapter_num"]: c["cp_earning_word_count"]
        for c in chapter_facts["chapters"]
    }
    ...
    for chap_num, nrolls in rolls_per_chapter.items():
        cp_words = cp_words_by_num.get(chap_num, 0)
        if cp_words == 0:
            continue
        title = full_title_by_num.get(chap_num, chap_num)
        expected_rolls = cp_words / 2000
        ratio_off = abs(nrolls - expected_rolls) / max(expected_rolls, 0.5)
        n_compared += 1
        if ratio_off > ROLL_PACE_TOLERANCE:
            n_off += 1
            big_misses.append((title, nrolls, cp_words, ratio_off))
    big_misses.sort(key=lambda x: -x[3])
    print(f"\ncheck 2 - roll pace (rolls x 2000 ≈ CP-eligible words):")
    print(f"  chapters compared:     {n_compared}")
    print(f"  off by >{int(ROLL_PACE_TOLERANCE*100)}%:           {n_off}")
    for title, nrolls, cp_words, ratio in big_misses[:10]:
        print(f"    {title[:60]:60s}  rolls={nrolls:2d}  cp_words={cp_words:>6d}  off={ratio:.0%}")
    if n_off:
        errors.append(
...
def check_perks_in_catalog(rolls: dict, catalog: dict) -> list[str]:
    ...
    print(f"\ncheck 3 - acquired perks vs catalog:")
...
def check_obtained_perks_alignment(chapters: dict, obtained: dict) -> tuple[list[str], list[str]]:
    ...
    bad_chap = sorted({p["chapter_num"] for p in obtained["perks"]} - chapter_nums)
    bad_seq = [p for p in obtained["perks"] if not (1 <= p["epub_sequence"] <= n_chaps)]
    print(f"\ncheck 4 - obtained_perks alignment:")
    ...
    if bad_chap:
        hard.append(f"  obtained_perks references {len(bad_chap)} chapter_num(s) not in chapters.json")
    if bad_seq:
...
def main() -> int:
    ...
    chapter_facts = _load("chapter_facts")

    hard: list[str] = []
    soft: list[str] = []

    hard.extend(check_chapter_alignment(chapters, rolls))
    soft.extend(check_roll_pace(chapters, rolls, chapter_facts))
