...
# Require a name (alphabetic) after the keyword. Avoids matching things
# like "28 Preamble - Addendum Sophia" where the word after "Preamble"
# is just a hyphen (the "Preamble" there is a section-label, not a
# character name).
...
def classify(section: dict, full_title: str) -> tuple[bool, str]:
    ...
    if header is None:
        if AN_RE.search(sample):
            return False, "implicit with author's-note marker"
        if word_count < 500:
